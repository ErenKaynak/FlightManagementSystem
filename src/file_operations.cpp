...
void Admin::loadFlightsFromFile() {
  ...
  while (getline(file, line)) {
    ...
    string flightId, origin, destination, date, depTime, arrTime, pilotId,
        aircraftId;
    string priceStr, durationStr;

    if (getline(iss, flightId, ',') && getline(iss, origin, ',') &&
        getline(iss, destination, ',') && getline(iss, date, ',') &&
        getline(iss, depTime, ',') && getline(iss, arrTime, ',') &&
        getline(iss, pilotId, ',') && getline(iss, aircraftId, ',')) {
      ...
      Flight flight(flightId, origin, destination, date, depTime, arrTime,
                    pilotId, ac, price, duration);
...
void Admin::saveFlightsToFile() {
  ...
  for (const auto &flight : flights) {
    file << flight.getFlightId() << "," << flight.getOrigin() << ","
         << flight.getDestination() << "," << flight.getDate() << ","
         << flight.getDepartureTime() << "," << flight.getArrivalTime() << ","
         << flight.getPilotId() << "," << flight.getAircraft().getAircraftId()
         << "," << flight.getPrice() << "," << flight.getDurationMinutes()
         << "\n";
  }
  file.close();
}
...
void Admin::loadAircraftsFromFile() {
  ...
  while (getline(file, line)) {
    ...
    if (getline(iss, aircraftId, ',') && getline(iss, model, ',') &&
...
void Admin::saveAircraftsToFile() {
  ...
  for (const auto &aircraft : aircrafts) {
    file << aircraft.getAircraftId() << "," << aircraft.getModel() << ","

...
void Admin::assignAircraftToFlight() {
  ...
  for (size_t i = 0; i < aircrafts.size(); i++) {
    if (isAircraftAvailable(aircrafts[i].getAircraftId(),
                            ...
                            selectedFlight.getArrivalTime(), flightIdx)) {
  ...
  if (availableAircrafts.empty()) {
    cout << "No aircrafts available at this time. All aircrafts are already "
            "assigned to other flights.\n";
    return;
  }
  ...
  if (aircraftChoice < 1 ||
      aircraftChoice > static_cast<int>(availableAircrafts.size())) {

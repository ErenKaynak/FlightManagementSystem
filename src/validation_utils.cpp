...
int Admin::compareTimes(string time1, string time2) {
    ...
    if (time1.empty() || time2.empty()) return 0;
...
bool Admin::isAircraftAvailable(string aircraftId, string date, string depTime, string arrTime, int excludeFlightIndex) {
    if (depTime.empty() || arrTime.empty()) return true; // No time conflict if times not set
    
    for (size_t i = 0; i < flights.size(); i++) {
        if (static_cast<int>(i) == excludeFlightIndex) continue;
        
        Flight& flight = flights[i];
        if (flight.getAircraft().getAircraftId() == aircraftId && flight.getDate() == date) {
            string flightDep = flight.getDepartureTime();
            string flightArr = flight.getArrivalTime();
            
            if (flightDep.empty() || flightArr.empty()) continue;
            
            // Check for time overlap
            // If new departure is between existing flight times, or
            // If new arrival is between existing flight times, or
            // If new flight completely contains existing flight
            if ((compareTimes(depTime, flightDep) >= 0 && compareTimes(depTime, flightArr) <= 0) ||
                (compareTimes(arrTime, flightDep) >= 0 && compareTimes(arrTime, flightArr) <= 0) ||
                (compareTimes(depTime, flightDep) <= 0 && compareTimes(arrTime, flightArr) >= 0)) {
                return false;
            }
        }
    }
...
bool Admin::isPilotAvailable(string pilotId, string date, string depTime, string arrTime, int excludeFlightIndex) {
    if (depTime.empty() || arrTime.empty()) return true; // No time conflict if times not set
